...
IniRead, inting, config.ini, section4, inting
...
;Gui, Add, Text, y+15, How racist are you?
;Gui, Add, Text,, Not Racist
;Gui, Add, Slider, yp x+5 vRacism w200 Range0-100 ToolTipLeft ToolTipRight TickInterval10, 50
;Gui, Add, Text, yp x+5, Very Racist
...
Gui, Add, CheckBox, vinting, Intentionally feed (EXPERIMENTAL! ONLY USE IN INTRO BOTS!)
GuiControl,, inting, %inting%
...
IniWrite, %inting%, config.ini, section4, inting
...
IniWrite, %A_Space%, config.ini, section4, prioritypick
IniWrite, 0, config.ini, section4, inting
...
loop
{
	...
	if (PickedChampion!=None)
	{
		...
		loop
		{
			...
			click, Right
			CoordMode, Mouse, Relative
			PixelSearch, ci, di, 552, 3, 552, 3, 0xceba6b, 5, Fast RGB
			if ErrorLevel=0
			{
				WinActivate, League of Legends (TM) Client ahk_exe League of Legends.exe
				SleepRandom(150)
...
;----------------------------------------------------------------------------------------------------------------------------------;
	...
	if inting=1
	{
		Loop
		{
			...
			GameDisconnectCheck(GameDisconnect) ;check for disconnect
	...
	loop
	{
		...
		{
			PixelSearch, ax, bx, 0, 0, 534, 316, 0xf2d65d,, Fast, RGB ;checks if your character is on screen
			if ErrorLevel=0
			{
		...
		loop, %LoopCount%
		{
			...
			if ErrorLevel=0
			{
				...
				PixelSearch, ax, bx, 260, 350, 260, 350, 0x010d07, 10, Fast RGB ;check hud hp bar if dead
				if ErrorLevel=0
				{
					AttemptShopping%PickedChampion%()
					SleepRandom(1200)
		...
		PixelSearch, ax, bx, 0, 0, 534, 316, 0x340300, 5, Fast, RGB ;check for enemies on screen
		if ErrorLevel=0
		{
